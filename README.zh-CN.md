...
控制台是**图标，而图标是推导出来的** —— `src/` 里不存任何一个主题的词汇表，自然
也不能为 `SKIRTS` 存一个图标。

**视图**的图标，是用相机那套 az/el 基向量把主体自身的包围盒从该视角画出来。长车体
在右视得到一个扁长方形、在主视得到一个窄长方形，因为它本来就是这样；正投影视图会带上
图纸页该有的点划线基准，剖视图会带上剖切迹线。作者自创的任何视图都免费得到一枚诚实的
图标。

**动作**的图标，是顺着 `motion.set` 查进 `channel.bind` 表达式、找出它真正驱动的
通道类型 —— 于是 `EXPLODE` 是发散、`DRIVE` 是旋转、`FIRE` 是喷射，没有人需要把这件事
写下来。画之前比例先开平方压缩过：图标是主体的符号，真实 4.2:1 的车体塞进 24px 只是
一条缝。

`view.icon` / `motion.icon` 可以覆盖推导结果。两个示例都没用 —— 这正是重点：如果
随仓库发布的 spec 需要它们，就说明推导不成立。

**两层标注可以关掉。** 序号标注和尺寸标注各是一个按钮：一张带十六个序号气泡和整套
尺寸的图是正确的，而当你正把东西翻过来看造型时，它也确实碍事。尺寸默认开启 —— 不带
尺寸的图纸页不是图纸页，质量闸口就是这么规定的。

取景随之联动。`0.78`（图形占整张图的比例）不是审美，而是气泡走廊占掉的宽度；关掉序号
就该把这块地还给图形。在窄而竖的屏幕上，这是占宽 79% 与 94% 的差别 —— 也正因如此，
序号是那里唯一默认**关闭**的东西：走廊是横向的，手机上宽度是唯一稀缺的轴，而其中五分之
一在没人开口之前就被留给了气泡。按一下就调回来，取景在后面平滑收回。读者一旦按过，这个
答案就跟着他走，之后任何尺寸变化都不会被布局改掉。

**三个信息栏都能折叠**，各自缩成所在那一角的一枚手柄；画面在动的时候它们会自己让位 ——
用的是标注层淡出的同一个时钟，所以信息栏和气泡是一起走、一起回，而不是两个几乎同步的
计时器各走各的。

在手机上每个栏目默认折叠，展开时占满阅读区；控制台变成底边一条横向滚动的工具条，每枚
...
dev/                  剖面线着色器实验台、布局实验台、无头截图、
                      平滑度 + 锚点 + 爆炸 + 触控探针、离线证据与调研检查、静态服务器
```
